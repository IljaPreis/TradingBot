...
app=FastAPI(title="TradingBot V110 Trade Manager Pro")
...
@app.get("/health")
async def health(): return {"status":"online","name":"TradingBot V110 Trade Manager Pro","markets":len(WATCHLIST)}
@app.get("/test-telegram")
async def test_telegram(): await tg("✅ TradingBot V110 Trade Manager Pro ist online."); return {"telegram":"sent"}
@app.get("/context")
...
async def webhook(req:Request):
    ...
    msg=f"""🚨 <b>TradingBot V110 Trade Manager Pro</b>
<b>{priority}</b>
...
async def v40_info():
    return {
        "version": "TradingBot V110 Trade Manager Pro",
        "features": [
            "V31 AI Score 0-100",
...
async def telegram_command(data: dict):
    ...
    if cmd == "/top":
        ...
        msg = f"✅ TradingBot V110 Trade Manager Pro online | Märkte: {len(WATCHLIST)} | Trades: {len(TRADES)}"
    else:
...
async def telegram_v110(data: dict):
    ...
    if cmd == "/manager":
        ...
        msg = f"✅ TradingBot V110 Trade Manager Pro online | Trades: {len(TRADES)}"
    else:
