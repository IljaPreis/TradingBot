...
app=FastAPI(title="TradingBot V500 Release Candidate")
...
@app.get("/health")
async def health(): return {"status":"online","name":"TradingBot V500 Release Candidate","markets":len(WATCHLIST)}
@app.get("/test-telegram")
async def test_telegram(): await tg("✅ TradingBot V500 Release Candidate ist online."); return {"telegram":"sent"}
@app.get("/context")
...
async def webhook(req:Request):
    ...
    msg=f"""🚨 <b>TradingBot V500 Release Candidate</b>
<b>{priority}</b>
...
async def v40_info():
    return {
        "version": "TradingBot V500 Release Candidate",
        "features": [
            "V31 AI Score 0-100",
...
async def telegram_command(data: dict):
    ...
    if cmd == "/top":
        ...
        msg = f"✅ TradingBot V500 Release Candidate online | Märkte: {len(WATCHLIST)} | Trades: {len(TRADES)}"
    else:
...
async def telegram_v110(data: dict):
    ...
    if cmd == "/manager":
        ...
        msg = f"✅ TradingBot V500 Release Candidate online | Trades: {len(TRADES)}"
    else:
...
async def telegram_v120(data: dict):
    ...
    if cmd in ["/smc", "/smartmoney"]:
        ...
        msg = f"✅ TradingBot V500 Release Candidate online | SMC Märkte: {len(SMART_MONEY)}"
    else:
