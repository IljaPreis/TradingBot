...
echo "== V7219-V7226 SAFE PERFORMANCE PACK =="
mkdir -p backups app data ops

echo "== Backup =="
tar -czf "backups/v7219_v7226_safe_before_${TS}.tar.gz" \
  --exclude='backups' \
  --exclude='__pycache__' \
  --exclude='.git' \
  . || true
...
echo "== Config =="
cat > data/v7219_v7226_performance_config.json <<'JSON'
{
  "version": "V7219-V7226",
  "enabled": true,
  "observe_only": true,
...
echo "== Module =="
cat > app/v7219_v7226_performance_learning_pack.py <<'PY'
...
if p.exists():
    ...
    routes = [
        "/performance-learning", "/setup-performance", "/market-session-performance",
        "/news-performance", "/shadow-edge", "/best-times", "/weak-setups",
        "/daily-performance-report",
    ]
    for route in routes:
        marker = f'    "{route}",'
        if marker not in s:
            s = s.replace('    "/daily-intelligence",\n}', f'    "/daily-intelligence",\n    "{route}",\n}}')
    if '/performance-learning?token=' not in s:
        s = s.replace(
...
echo "== main hook =="
if ! grep -q "V7219 V7226 PERFORMANCE LEARNING PACK INSTALL" app/main.py; then
cat >> app/main.py <<'PY'
...
echo "== ops script =="
cat > ops/v7226_daily_performance_report.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
...
cd /opt/tradingbot_v6000 || exit 1
mkdir -p data
curl -fsS "${BASE_URL}/daily-performance-report.json?token=${TOKEN}&write=1" > data/v7226_daily_performance_report_last.json
python3 -m json.tool data/v7226_daily_performance_report_last.json >/dev/null
echo "v7226 daily performance ok $(date -u +%Y-%m-%dT%H:%M:%SZ)"
SH
chmod +x ops/v7226_daily_performance_report.sh

echo "== cron =="
cat > /etc/cron.d/tradingbot_v7226_daily_performance <<'CRON'
SHELL=/bin/bash
PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
17,47 * * * * root cd /opt/tradingbot_v6000 && /opt/tradingbot_v6000/ops/v7226_daily_performance_report.sh >> /opt/tradingbot_v6000/data/v7226_daily_performance_cron.log 2>&1
CRON
chmod 0644 /etc/cron.d/tradingbot_v7226_daily_performance
...
if [ -f "$CHECK_FILE" ] && ! grep -q "V7219-V7226 PERFORMANCE LEARNING PACK ROUTES" "$CHECK_FILE"; then
cat >> "$CHECK_FILE" <<'EOF'

echo ""
echo "===== V7219-V7226 PERFORMANCE LEARNING PACK ROUTES ====="
TOKEN_FOR_V7226="eHwFukO31kypn0KZenWjht2T815BlQeeZNygm9nUwTg"
for U in performance-learning performance-learning.json performance-learning-config.json setup-performance setup-performance.json market-session-performance market-session-performance.json news-performance news-performance.json shadow-edge shadow-edge.json best-times best-times.json weak-setups weak-setups.json daily-performance-report daily-performance-report.json; do
  echo "/${U}?token=*** -> $(curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1/${U}?token=${TOKEN_FOR_V7226}")"
done
echo "daily_performance_cron -> $(test -f /etc/cron.d/tradingbot_v7226_daily_performance && echo OK || echo MISSING)"
echo "daily_performance_file -> $(test -f data/v7226_daily_performance_report_last.json && echo OK || echo MISSING)"
for R in performance-learning setup-performance market-session-performance news-performance shadow-edge best-times weak-setups daily-performance-report master markets single-header daily-intelligence; do
  TMP="/tmp/v7226_${R}.html"
  CODE="$(curl -s -o "$TMP" -w "%{http_code}" "http://127.0.0.1/${R}?token=${TOKEN_FOR_V7226}")"
  HEADER="$(grep -c 'id="v7208_single_header_bar"' "$TMP" || true)"
  PERF="$(grep -c '/performance-learning' "$TMP" || true)"
  echo "/${R}?token=*** -> ${CODE} | single_header_hits=${HEADER} | performance_link_hits=${PERF}"
done
EOF
chmod +x "$CHECK_FILE"
fi
...
echo "== syntax =="
python3 -m py_compile app/v7219_v7226_performance_learning_pack.py app/v7208_single_header_mode.py app/main.py

echo "== docker rebuild =="
docker compose up -d --build tradingbot || docker restart tradingbot
sleep 6

TOKEN_TEST="eHwFukO31kypn0KZenWjht2T815BlQeeZNygm9nUwTg"
...
for R in performance-learning setup-performance market-session-performance news-performance shadow-edge best-times weak-setups daily-performance-report master markets single-header daily-intelligence; do
  ...
  HEADER="$(grep -c 'id="v7208_single_header_bar"' "$TMP" || true)"
  PERF="$(grep -c '/performance-learning' "$TMP" || true)"
  echo "/${R}?token=*** -> ${CODE} | single_header=${HEADER} | performance_link=${PERF}"
done

echo ""
echo "== Daily Performance Snapshot =="
bash ops/v7226_daily_performance_report.sh "$TOKEN_TEST" || true
echo "daily_performance_file=$(test -f data/v7226_daily_performance_report_last.json && echo OK || echo MISSING)"
echo "daily_performance_cron=$(test -f /etc/cron.d/tradingbot_v7226_daily_performance && echo OK || echo MISSING)"

echo ""
echo "== Performance Preview =="
curl -s "http://127.0.0.1/performance-learning.json?token=${TOKEN_TEST}" \
  | python3 -m json.tool \
  | grep -E '"safe_state"|"risky_on"|"rows_total"|"best_setups"|"weak_setups"|"best_shadow_edges"|"best_sessions"|"candidate_inbox"|"current_event_risk"' \
  | head -n 100 || true
...
echo "single_header_http=$(curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1/single-header?token=${TOKEN_TEST}")"
echo "daily_intelligence_http=$(curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1/daily-intelligence?token=${TOKEN_TEST}")"

echo ""
echo "== V7219-V7226 DONE =="
